.PHONY: help sync sync-dev install build publish publish-test test test-unit test-e2e lint format format-md clean \
        install-hooks version-patch version-minor version-major stack-up stack-down
...
stack-up: ## Start local test stack (relay + signer)
...
dry-run: ## Dry-run publish example (requires test fixture)
	uv run nostr-publish tests/integration/fixtures/test-article.md --relay ws://localhost:8081 --dry-run
